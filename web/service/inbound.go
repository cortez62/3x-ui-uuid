...
func (s *InboundService) GetClientExpiryByUUID(uuid string) (int64, bool, error) {
	...
	result := db.Raw(`
		SELECT CAST(JSON_EXTRACT(client.value, '$.expiryTime') AS INTEGER) AS expiry_time
		FROM inbounds,
			JSON_EACH(JSON_EXTRACT(inbounds.settings, '$.clients')) AS client
		WHERE JSON_EXTRACT(client.value, '$.id') = ?
		LIMIT 1
	`, uuid).Scan(&row)
	if result.Error != nil {
	...
	return row.ExpiryTime, true, nil
}
